...
// Requête SQL pour vérifier si la clé d'inscription existe dans la table admin
$sql = "SELECT * FROM cle WHERE cle_inscription = '$cle_inscription'";
$result = $conn->query($sql);

// Vérifier si la requête a retourné des résultats
if ($result->num_rows > 0) {
    // La clé d'inscription est valide, rediriger l'utilisateur vers la page d'inscription
    header("Location: inscription-responsable.php");
    exit();
} else {
